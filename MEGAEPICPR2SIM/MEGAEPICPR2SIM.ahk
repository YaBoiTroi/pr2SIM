...
SetKeyDelay, (delay//2), 0 ; delay after text input/key press
...
macroExperimental(){
	...
	Sleep, 25
	ControlSend,, {Up down}{Down up}, % "ahk_id " . IDs[2] 
	;FindThisText("|<>*102$10.7Uz7yzzzzzzxzby7U000000007Vzbyzzzzzzxzby7U000000007Vzbyzzzzzzxzby7W", IDs[2],,,,.5,, "gun wait 2 instance 2",,,,,false,,,,,false) ; wait for gun reload
	;Sleep, delay + 150
	;if(reboot){
	;	return
	;}
	;ControlSend,, {Left Down}{Space down}, % "ahk_id " . IDs[2] ;
	;Sleep, 50 + (delay//5)													 ; gun back 2 (repeat)
	;ControlSend,, {Left Up}{Space up}, % "ahk_id " . IDs[2] 	 ;
	Sleep, 300 + (delay * 5)
	ControlSend,, {Up up}{Right up}, % "ahk_id " . IDs[2] ; stop move 2
	;Sleep, delay + 10
	Sleep, 300 + (delay*5)
	FindThisText("|<>*121$18.S0+n02V9DV9+V9+Z9+r9+Tj/U", IDs[2],,, True,5, delay + 50, "quit 2",,.05) ;quit 2
	if(reboot){
		return
	}
	...
	Sleep, 50
	ControlSend,, {Up down}{Down up}, % "ahk_id " . IDs[3] 
	...
	;Sleep, 50 + (delay//5)													 ; gun back 3 (repeat)
	;ControlSend,, {Left Up}{Space up}, % "ahk_id " . IDs[3] 	 ;
	Sleep, 375 + (delay * 5)
	...
	Sleep, 50
	ControlSend,, {Up down}{Down up}, % "ahk_id " . IDs[4] 
	Sleep, 100
	if(simType=1){
		Loop, 3{
			if(FindText("wait", .5, 0, 0, 0, 0, 0, 0,"|<>*102$10.7Uz7yzzzzzzxzby7U000000007Vzbyzzzzzzxzby7U000000007Vzbyzzzzzzxzby7W")){ ; wait for gun reload
				Sleep, 100 + delay
		...
		FindText("wait", 5, 0, 0, 0, 0, 0, 0,"|<>*141$29.U0UU1011023nnqA4YYYc99ddEGHHCUYYYNxltsk0001000068")
	}
...
levelPrep(){
    Loop, 4{
        loopID:=IDs[A_Index]
        FindThisText(Text:="|<>*230$13.1k7z7zrzzzzzzzzzzzzzzzzyzy7w1w8", loopID,,,,5, (delay*5) + 100, "hat menu") ; click (?) for hats
        if(reboot){
			return
		}
		...
		FindThisText(Text:="|<>*152$25.w00UE0008D9/rYYZ+4GGb299HUYYdSSSLU10200U12", loopID,,,,5, (delay*10) + 500, "equip exp hat") ; click equip
		if(reboot){
			return
		}
	...
	/*
		...
		Sleep, delay + 100
		FindThisText("|<>*153$14.S8wmG4cVC8Gm4Yn8bWC", loopID,,,,5, delay + 100, "confirm hat color") ; OK button for hat color
		if(reboot){
			return
		}
		...
		FindThisText(Text:="|<>*202$56.zzzzzzzzzk000000004000000001000000000E000000004000000001000000000E000000004000000001000000000E000000004000000001000000000E000000004000000001000000000E000000004000000001000000000E000000004000000001000000000M", loopID,2,,,5, delay + 100, "click color Hex Text box") ; color hex text
		if(reboot){
			return
		}
		ControlSend,, {text}#000000, % "ahk_id " . loopID ; black color
	...
	*/
	...
	/* 
		...
		Sleep, delay + 50
	*/
		FindThisText("|<>*186$25.000E000800047ltvwAlZa2FUVT8kEkYM8Mm6IDt1u6", loopID,,,,5, delay + 10, "search tab") ; search tab
		if(reboot){
			return
		}
